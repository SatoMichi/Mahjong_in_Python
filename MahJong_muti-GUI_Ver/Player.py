...
# function for socket programming
    def setConnection(self,conn):
        self.conn = conn
    
    def printS(self,s):
        self.gm.sendPlayer(self,s,"print")

    def inputS(self,s):
        self.gm.sendPlayer(self,s,"input")
...
# function for check 和
    ...
    def askRiichi(self):
        ...
        if cutpais != [] :
            # interaction 
            i = int(self.inputS("You can 立直! If you Do not want to 立直 Please press '0'\n Which Pai do you want to cut? (1~) \n"+Pai.showHand(cutpais)+": "))
            if i == 0:
                self.isRiichi = False
            else:
                self.isRiichi = True
    ...
    def askAnKang(self):
        ...
        if len(posibleMinset)==0:
            ...
            ankang = self.inputS("Do you want to 暗槓"+Pai.showHand(posibleMinset[0])+"? y/n: ")
            if ankang == "y":
                self.minset = posibleMinset[0]
                self.printS("You selected {}".format(Pai.showHand(self.minset)))
                return True
            else:
                return False
        else:
            ...
            if ankang == "y":
                idx = int(self.inputS("Which set do you want to 暗槓? Please input the number(1~) \n"
    ...
    def askChi(self,posibleMinset):
        minchi = self.inputS("Do you want to 吃? y/n: ")
        if minchi ==  "y":
            chi_prompt = "Select from: \n"
            for i, option in enumerate(posibleMinset):
                chi_prompt += "{}. {} ".format(i+1, Pai.showHand(option))
            select = self.inputS(chi_prompt)
            self.printS("You selected {}".format(Pai.showHand(posibleMinset[int(select)-1])))
    ...
    def askPon(self,posibleMinset):
        minpon = self.inputS("Do you want to 碰"+Pai.showHand(posibleMinset)+"? y/n: ")
        if minpon == "y":
    ...
    def askMinKang(self,posibleMinset):
        ...
        if len(posibleMinset)==0:
            ...
            minkang = self.inputS("Do you want to 槓"+Pai.showHand(posibleMinset)+"? y/n: ")
            if minkang == "y":
                self.minset = posibleMinset
                return True
            else:
